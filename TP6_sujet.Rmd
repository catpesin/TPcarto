...
##Question 1 
*italique*  **gras** markdownguide.org cheat-sheet 
Insérer lateX
$$
a\times x = b
$$
...
geom_vline(xintercept=mean(marseille$DISP_MED18))
...
#Moyenne des revenus disponibles médians par arrondissement est significative
#pvaleur du test de fisher ~ 0

summary(aov(DISP_MED18~depcom,data=marseille))
#Attention : la normalité de revenu n'est pas assurée
```
...
```{r}
marseille<-marseille %>% 
  filter(!is.na(DISP_MED18)) #drop_na() possible aussi
...
#La distribution des revenus médians n'est pas aléatoire. Le phénomène semble spatialement corrélé.
```
...
```{r}
#Les individus qui sont voisins ont tendance à avoir des valeurs proches -> autocorrélation positive
```
...
str(voisins)#on obtient une liste de longueur 336 (nbre d'iris sans na)
summary(voisins)#4 iris ont 1 voisin, ...
```
...
```{r}
#Poids=1 pour les voisins, poids=0 pour les autres
#On pourrait pondérer par un poids inversement proportionnel à la distance
...
Le second argument à préciser (`listw`) correspond à la liste des poids des voisins que vous avez créée précédemment.(probleme d'affichage->dev.off)
...
#La plupart des points sont dansles quartiers low-low ou high-high
```
...
d. D'après le diagramme de Moran, les revenus médians semblent-ils autocorrélés spatialement ? Si oui, l'autocorrélation vous semble-t-elle positive ou négative ?#positive
...
#L'indice se situe généralement entre -1 et 1 (0 : pas de corrélation)
#ici 0.71 pvaleur quasi-nulle : H0 rejetée, il y a autocorrélation
moran.test(marseille$DISP_MED18_ALEA, ponderation, randomisation = TRUE)
#ici indice proche de 0
```
b. Interpértez le résultat obtenu : confirme-t-il ou non votre hypothèse ?

L'indice est proche de 1, ce qui confirme notre hypothèse : il y a autocorrélation positive du phénomène.
